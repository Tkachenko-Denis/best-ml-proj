...
1. Никогда не коммитьте `terraform.tfvars` с реальными токенами
...
### Ошибка: "Error: Error while requesting API"
- Проверьте правильность токена и ID каталога
- Убедитесь, что у вас есть права на создание ресурсов

### Ошибка: "quota exceeded"
- Проверьте квоты в веб-консоли
- Запросите увеличение квот или удалите неиспользуемые ресурсы

### ВМ создалась, но SSH не работает
- Проверьте, что SSH ключ указан правильно
- Убедитесь, что Security Group разрешает порт 22
- Подождите 1-2 минуты после создания ВМ

## Следующие шаги

После создания инфраструктуры можно:

1. Настроить ВМ с помощью Ansible (см. `ansible/` директорию)
2. Развернуть FastAPI приложение
